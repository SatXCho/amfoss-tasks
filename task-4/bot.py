...
def getMovie(message):
    ...
    if os.path.exists("tasks/task-04/movies.csv") == False:
        file = open("tasks/task-04/movies.csv", "w")
        ww = csv.writer(file)
        ww.writerow(apidict.keys())
        file.close()

    with open("tasks/task-04/movies.csv", "a") as csvfile:
...
def getList(message):
    ...
    sentcsv = open("tasks/task-04/movies.csv", 'rb')
